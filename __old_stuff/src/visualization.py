...
def fitness_over_time(trial_ID):
    ...
    s_dev = [sqrt(sum([(fits[i][j]-means[i])**2 for j in range(lfi)])/(lfi-1)) 
                                  for i in range(lf)]
    # Plot all:
    pylab.plot( [j+2 for j in range(len(fits))], fits, 'b.')
    pylab.xlabel('number of neurons in population')
    pylab.ylabel('fitness')
    #pylab.plot(indices, means, 'r-')
    #pylab.plot(indices, [means[i]-s_dev[i] for i in range(lf)], 'g-')
    #pylab.plot(indices, [means[i]+s_dev[i] for i in range(lf)], 'g-')
    pylab.hold(False)
    pylab.title('Fitness of initial populations of differently sized networks.')
    pylab.show()
    pylab.cla()
